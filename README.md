...
| `forge-security-audit` | **Whole-codebase vulnerability hunt** (vendored from Cloudflare's [security-audit-skill](https://github.com/cloudflare/security-audit-skill), MIT): six phases — reconnaissance into a deterministic coverage ledger, coverage-led hunting by isolated agents across core + domain attack classes (web/auth, client-side, AI/LLM, supply chain, cloud, RPC/messaging, resource exhaustion, data isolation, desktop/mobile, memory safety), adversarial validation of every candidate by a fresh agent, schema-validated `findings.json` (`confirmed` / `needs_validation` / `rejected`, severity only on confirmed), independent record verification, target-neutral report. Profiles `quick` / `standard` / `deep`, scoped runs, agent budgets, additive re-runs. `--plan` runs reconnaissance only and prints ranked scan targets (`plan-rank.cjs`: tool hits, 90-day churn, low-trust surfaces, estimated agents per target) so a big codebase is scanned boundary by boundary, recon paid once. Seeds recon from `architecture.md`'s Trust Boundaries and `security-check.sh`; records to the review ledger; routes fixes to `forge-fix` and regex-catchable misses to `security-rules.txt`. Guidance mode for plain security questions. Read-only on source; needs Node ≥ 18 for the two zero-dep validators. Where `forge-gatekeeper` gates a release diff in minutes, this hunts the whole codebase in hours. |
...
├── skills/                  # the 26 forge-* skills (+ bundled templates,
│   └── .../                 #   references, detect/migrate scripts)
├── agents/                  # 10 model-pinned subagents
├── hooks/                   # hooks.json + zero-token shell scripts
├── dashboard/               # bundled forge-office web dashboard (zero-dep Node, read-only)
