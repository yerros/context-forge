...
- Sprites (0.38.0, redesigned office 0.39.0): append `?sprites=1` to the
  dashboard URL for the full redesigned office — original sprite characters
  (`dashboard/public/sprites.png`, generated by
  `dashboard/sprites/forge_sprites.py`, no third-party assets) plus the new
  scene: brick wall with neon FORGE sign, central glass meeting room, oak
  parquet, zone rugs + sign badges, server rack, coffee bar, lounge, warm
  lighting. Without the flag the classic procedural office renders unchanged.
  Regenerate the atlas with `python3 dashboard/sprites/forge_sprites.py`
  (needs Pillow).
