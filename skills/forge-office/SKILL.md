...
- Sprites (0.38.0): append `?sprites=1` to the dashboard URL to render the ten
  forge agents as original pixel-art sprites (`dashboard/public/sprites.png`,
  generated by `dashboard/sprites/forge_sprites.py` — no third-party assets).
  Without the flag the procedural office renders unchanged. Regenerate the atlas
  with `python3 dashboard/sprites/forge_sprites.py` (needs Pillow).
